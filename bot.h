...
public:
    Bot(uint32_t intents) : dpp::cluster("OTQ2ODM2Mzg4MTkwNDk4ODU2.YhkgGg.szcUNFly3moCylBdaoijIiojdic", dpp::intents::i_default_intents | intents), m_rDistribution(0, 10)
    {
        ...
        //m_hCmdHandler = std::bind(cmd_clippy, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
        ...
        on_ready([&cmd_handler, this](const dpp::ready_t& event)
        {
            ...
            play_args.push_back(std::make_pair<std::string, dpp::param_info>("URL", dpp::param_info(dpp::pt_string, false, "play")));
            ...
            cmd_handler.add_command("play", play_args, std::bind(&Bot::CmdPlay, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), "Play stuff");
...
/*         on_voice_state_update([this](const dpp::voice_state_update_t& event)
    ...
    std::vector<uint8_t> ReadAudioData(const std::string&);
    ...
    void CmdStop(const std::string&, const dpp::parameter_list_t&, dpp::command_source);
    void CmdSummon(const std::string&, const dpp::parameter_list_t&, dpp::command_source);
    void PlayYoutube(dpp::discord_voice_client*);
